...
- Claude simula respostas do cliente turn-by-turn, usando o SYSTEM_PROMPT **literal** desta skill.
- Quando aluno disser `fim` (ou após N trocas), Claude gera relatório estruturado com score 0-100.
- Sessão salva em `~/minha-maquina-de-vendas/treino-vendas/sessao-{YYYY-MM-DD-HHMM}.md`.
...
O perfil do lead emerge de `scenario_brief` (segmento + problema + orçamento + urgência + decisor + objetivo) combinado com `difficulty` (`hard` → "mais resistente e desconfiado"). Não criar uma lista fixa de perfis; coletar os 6 campos do `scenario_brief`.
...
Path: `~/minha-maquina-de-vendas/treino-vendas/sessao-{YYYY-MM-DD-HHMM}.md`
...
### 6. Confirmar o arquivo local

Usar a tool Write do Claude Code para salvar somente o Markdown em `~/minha-maquina-de-vendas/treino-vendas/sessao-{YYYY-MM-DD-HHMM}.md` e informar esse caminho ao aluno junto do score.
...
- NO relatório, se Claude não devolver JSON parseável, usar o fallback definido nesta skill (score 50 + texto "Análise inconclusiva").
